    ...
    if (isFunction(next[0])) {
      ...
      if (newNode instanceof Fragment) {
        if (isBegin(target)) {
          ...
          let c = target.__length;
          let i = target.__self.offset;
          while (c-- > 0) {
            q.push(target.parentNode.childNodes[i++]);
          }

          target.__length = newNode.length;
          newNode.childNodes.forEach(node => {
            target.parentNode.insertBefore(node, target.__self.end);
          });
      ...
      if (target.nodeType === 3) {
        if (isBegin(target)) {
          ...
          if (isBlock(prev) && isBlock(next)) {
            ...
            target.nodeValue = [].concat(next).join('');
          }
        }
      } else {
        target = await upgradeNode(target, prev, next, svg, cb);
